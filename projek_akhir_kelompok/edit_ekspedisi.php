                            ...
                            <div class="card p-3">
                                <div class="card-body p-0">
                                    <div class="heading mb-5">
                                        <h2 class="text-center">Update Ekspedisi</h2>
                                        <h6 class="text-center mb-4">Edit Pilihan Ekspedisi</h6>
                                        <hr>
                                    </div>
                                    <?php
                                    include "sistem/koneksi.php";
                                    $eeks = $_GET['eeks'];
                                    $query = "SELECT * FROM ekspedisi where id_eks = '$eeks'";
                                    $sql = $connect->query($query);
                                    $data = $sql->fetch_object();
                                    ?>
                                    <form action="sistem/update_ekspedisi.php" method="POST">
                                        <div class="row">
                                            <div class=" form-group mb-3 col-6">
                                                <input type="text" class="col form-control"
                                                    placeholder="Masukkan Nama Ekspedisi" name="jenis"
                                                    value="<?=$data->jenis?>">
                                            </div>
                                            <div class=" form-group mb-3 col-6">
                                                <input type="number" class="col form-control"
                                                    placeholder="Masukkan Biaya Ekspedisi" name="biaya"
                                                    value="<?=$data->biaya?>">
                                            </div>
                                            <div class="input-form mb-4">
                                                <button name="id" value="<?=$eeks?>" type="submit"
